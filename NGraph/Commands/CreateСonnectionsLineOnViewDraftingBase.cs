...
namespace NGraph.Commands;
    ...
    public abstract class CreateСonnectionsLineOnViewDraftingBase : ExternalCommand
    {
        ...
        // === Точка расширения: по умолчанию — ничего не делаем ===
        /// <summary>
        ///     Устанавливает дополнительные параметры зелёной точки (например, длину кабеля).
        ///     Переопределяется в наследниках, если нужен расчёт.
        /// </summary>
        protected virtual void ApplyAdditionalParameters(FamilyInstance fi, Element begin_element, Element end_element)
        {
            // no-op
        }
        ...
        FamilyInstance TransactionCreateCabel(ViewDrafting activeViewDrafting, VertexId begin_vertexId, VertexId end_vertexId, int numberInBus, int numberCabel)
        {
            ...
            // >>> Здесь подключается вариант поведения <<<
            ApplyAdditionalParameters(fi, begin_element, end_element);
